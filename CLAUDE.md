...
2. [CODE QUALITY] Component size and single responsibility — split by file:
   ...
   - 2f. `App.jsx` (2,579 lines) — worst offender; `GTDManager` mixes state, data flow, event handling, and rendering all at once. First pass: extract the six JSX rendering sections into purpose-built components (ErrorBoundary, ResizeHandle, AuthGate, AppSidebar, TaskBucketView, CoachPanel). Target: App.jsx ~1,650 lines; GTDManager return shrinks from ~582 lines to ~50.
   - 2g. `App.jsx` — second pass (callback extraction). After 2f, `GTDManager` still owns ~420-line `callAI`, inbox processing, project review, task CRUD, and all useEffect chains. Extract into focused custom hooks: `useCallAI` (AI fetch + tool dispatch), `useInboxProcessing` (processNextInboxItem / handleConfirmMove / startProcessInbox), `useTaskCrud` (add / move / delete / complete / recurrence), `useDragDrop` (project drag-and-drop), `useSupabaseSync` (the Supabase read/write/realtime useEffect chains). Each hook receives only the state setters it needs. `GTDManager` becomes a thin coordinator: invoke hooks, assemble context values, return JSX.
