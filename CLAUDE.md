...
~~21. Collapsible task list in Calendar view~~ — **done** (already implemented): both `CalendarPendingTasksSection` and `CalendarNewEventsSection` in CalendarManagementSections.jsx have identical collapse toggle (`useState(true)` + clickable header with ▾/▸).
...
~~24. Increase lightness of inactive buttons~~ — **done** (e838488): `Btn` idle background raised surface2 → surface3; `ToolbarBtn` component added in TaskBucketView with transparent → surface3 hover; Projects/GroupBy toolbar buttons migrated to ToolbarBtn; GroupBy active state uses inbox yellow text/border; idle text raised muted → text2 across toolbar and GroupBy buttons.
