...
| 2026-07-15 | Bug Fix | Issue#45 | GH#236 | Magic-link email not arriving on production - root cause: Supabase built-in mailer capped at 2 emails/hour; configured custom SMTP via smtp.gmail.com and raised rate_limit_email_sent to 30/hour (Supabase Auth config, not code); added error handling to sendMagicLink() in useSupabaseAuth.js so send failures surface in AuthGate UI instead of failing silently; tested pass on local dev and gtd.reitzersmith.com | a13983f |
