...
function ProjectTree({ parentId, depth, dragId, dropTarget, onDragStart, onDragOver, onDragEnd, onDrop, visibilitySet }) {
  const { allTasks, collapsedNodes, projectCategoryFilter, uncategorizedProjectId, showCompletedInProjects, showWaitingInProjects, showSomeDayInProjects } = useContext(TaskRowContext);
  if (depth > 6) return null;
  ...
  if (!showWaitingInProjects) children = children.filter(t => !t.isWaitingFor);
  if (!showSomeDayInProjects)  children = children.filter(t => !t.isSomeday);
